...
from ..models import Exam, ExamCreate, ExamUpdate, Question, Question_Tags, Answer_Choice, Exam_Question, Exam_Answer
...
def create_exam(*, session: SessionDep, exam_in: ExamCreate):
    ...
    # Randomly select the specified number of questions (or all if fewer available)
    ...
    # Create exam_question entries with positions
    exam_questions = []
    ...
    # Fetch answer choices for each selected question
...
def update_exam(*, session: SessionDep, exam_id: uuid.UUID, exam_in: ExamUpdate):
    ...
    # Get already answered question IDs (preserves timing data for future feature)
    ...
    for answer_input in exam_in.questions:
        ...
        # Only insert if this question hasn't been answered yet
    ...
    # If complete, calculate score with optimized query
    if exam_in.is_complete:
        # Count total questions and correct answers in one query
        result = session.exec(
    ...
    return {"message": "Exam answers saved successfully", "exam_id": exam_id}
...
@router.delete('/user/{member_id}/all')
def soft_delete_all_user_exams(member_id: str, session: SessionDep):
    # Get all non-deleted exams for the user
    exams = session.exec(
        select(Exam)
        .where(Exam.member_id == member_id)
        .where(Exam.deleted_at.is_(None))
    ...
    if not exams:
        return {"message": "No exams found for this user"}
    ...
    return {"message": f"Soft deleted {len(exams)} exams successfully"}


@router.delete('/{exam_id}/hard')
def hard_delete_exam(exam_id: uuid.UUID, session: SessionDep):
    ...
    # Hard delete exam (cascading deletes will handle exam_questions and exam_answers)
    session.delete(exam)
    session.commit()

    return {"message": "Exam permanently deleted"}


@router.delete('/user/{member_id}/all/hard')
def hard_delete_all_user_exams(member_id: str, session: SessionDep):
    # Get all exams for the user (including soft deleted)
    exams = session.exec(
        select(Exam).where(Exam.member_id == member_id)
    ).all()

    if not exams:
        return {"message": "No exams found for this user", "count": 0}

    # Hard delete all exams
    for exam in exams:
        session.delete(exam)

    session.commit()

    return {"message": f"Permanently deleted {len(exams)} exams", "count": len(exams)}
